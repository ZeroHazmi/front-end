...
const openai = new OpenAI({
	apiKey: process.env.OPENAI_API_KEY, // Ensure the key is in your environment
});
	...
	try {
		...
		if (!reportContent || !reportTypeName) {
			return NextResponse.json(
				{ error: 'Missing reportContent or reportTypeName' },
				{ status: 400 },
			);
		}
		...
		return NextResponse.json(
			{ error: 'Error analyzing report content' },
			{ status: 500 },
		);
	}
}
